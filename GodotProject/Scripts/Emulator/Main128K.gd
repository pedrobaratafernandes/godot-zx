...
# This script controls the ZX Spectrum 128K emulator scene.
# It bridges the GDExtension node (ZXEmulator128K) with Godot's UI and Input systems.
...
func _load_assets():
	# 1. ROM Loading
	# 128K models require external ROM files (32KB combined or 2 x 16KB banks).
	var rom_data = Config.get_128k_rom_data()
	if rom_data.size() > 0:
		emulator.load_rom(rom_data)
		print("128K ROM loaded successfully (Size: ", rom_data.size(), " bytes)")
	else:
		push_error("Error: 128K ROM not found. Please place '128.rom' (32KB) or 'ROM_128_0' and 'ROM_128_1' (16KB each) in res://roms/")
	...
	if FileAccess.file_exists(game_path):
		...
		match ext:
			"tap":
				...
				print("128K Tape loaded: ", game_path)
			"sna", "z80":
				emulator.load_snapshot(data)
				print("128K Snapshot loaded: ", game_path)
			"szx":
				push_warning("SZX format not supported in this build (requires rustzx-core 0.17.0+). Use .SNA or .Z80.")
			"scr":
				print("128K Screen dump preview only (full loading requires rustzx-core 0.17.0+): ", game_path)
			_:
