...
# It is designed to be used when exporting a single game as its own executable.
# Unlike the main version, it doesn't use the Launcher; the game path is defined via @export.
	...
	if game_path != "" and FileAccess.file_exists(game_path):
		...
		match ext:
			"tap":
				...
				print("Standalone 48K Tape loaded: ", game_path)
			"sna", "z80":
				emulator.load_snapshot(data)
				print("Standalone 48K Snapshot loaded: ", game_path)
			"szx":
				push_warning("SZX format not supported in this build. Use .SNA or .Z80.")
			"scr":
				print("Standalone 48K Screen dump preview only: ", game_path)
			_:
