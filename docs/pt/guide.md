...
1.  **Versão Launcher (Multi-jogo)**: Utilize `Scenes/Launcher/Launcher.tscn` como cena principal. Permite navegar nos jogos da pasta `res://games/`.
...
*   **Mapeamento de Input**: Traduz as ações de input do Godot (teclado, comando ou rato) para a matriz de teclas clássica do ZX Spectrum.
*   **Mapeamento de Setas Avançado**: Suporte para **Perfis de Mapeamento Duplo** (QAOP vs Sinclair) alternáveis via **Caps Lock**.
...
*   **Otimização .TAP**: O emulador está agora focado e otimizado para ficheiros **.tap** para garantir a máxima fidelidade no carregamento.
...
*   **Mobile vs Desktop**: Adaptação automática da UI (esconde controlos virtuais no PC).
